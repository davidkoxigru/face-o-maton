...
namespace face_o_maton
{
        ...
        where T : class
    {
        ...
        {
            var conf = endpointConf ?? GetEndpointFromConf(x => typeof(T).FullName.EndsWith(x.Contract)).Name;
        ...
        /// <returns>End-point read from configuration file</returns>
        public static ChannelEndpointElement GetEndpointFromConf(Func<ChannelEndpointElement, bool> predicate)
        {
            ...
            if (csg != null)
            {
                ...
                if (css is ClientSection)
                {
                    var cs = (ClientSection)csg.Sections["client"];
                    return cs.Endpoints.Cast<ChannelEndpointElement>()
